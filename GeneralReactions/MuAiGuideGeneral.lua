		...
		data = 
		{
			actions = 
			{
				...
				
				{
					...
					{
						aType = "Lua",
						actionLua = "if MuAiGuide and MuAiGuide.UI.open then\n    GUI:SetNextWindowSize(200, 0, GUI.SetCond_Appearing)\n    MuAiGuide.UI.visible, MuAiGuide.UI.open = GUI:Begin(\"指路绘制设置\", MuAiGuide.UI.open)\n    GUI:Dummy(1, 1)\n    GUI:Separator()\n    GUI:BulletText(\"玩家当前职能\")\n    GUI:SameLine()\n    GUI:TextColored(255, 0, 0, 1, \"[\" .. MuAiGuide.SelfPos .. \"]\")\n    GUI:SameLine()\n    GUI:Button(\"重新加载小队列表\")\n    if GUI:IsItemClicked(0) then\n        MuAiGuide.LoadParty()\n    end\n    GUI:Separator()\n    GUI:TextColored(0, 255, 0, 1, \"   拖动角色名进行职能修改\")\n    GUI:Separator()\n    GUI:Text(\" 职能位置    职业      角色名\")\n    if MuAiGuide.UI.visible then\n        if MuAiGuide.Party == nil or table.size(MuAiGuide.Party) == 0 then\n            MuAiGuide.Party = {}\n            MuAiGuide.Party.MT = nil\n            MuAiGuide.Party.ST = nil\n            MuAiGuide.Party.H1 = nil\n            MuAiGuide.Party.H2 = nil\n            MuAiGuide.Party.D1 = nil\n            MuAiGuide.Party.D2 = nil\n            MuAiGuide.Party.D3 = nil\n            MuAiGuide.Party.D4 = nil\n        end\n        GUI:PushItemWidth(283)\n        GUI:ListBoxHeader(\"##Jobs\", 284, 200)\n\n        local partyMembers = {\n            { info = MuAiGuide.Party.MT, label = \"MT\" },\n            { info = MuAiGuide.Party.ST, label = \"ST\" },\n            { info = MuAiGuide.Party.H1, label = \"H1\" },\n            { info = MuAiGuide.Party.H2, label = \"H2\" },\n            { info = MuAiGuide.Party.D1, label = \"D1\" },\n            { info = MuAiGuide.Party.D2, label = \"D2\" },\n            { info = MuAiGuide.Party.D3, label = \"D3\" },\n            { info = MuAiGuide.Party.D4, label = \"D4\" }\n        }\n        for i, member in ipairs(partyMembers) do\n            local IsSelected = function()\n                if member.label == MuAiGuide.Party.selected then\n                    return true\n                end\n                return false\n            end\n            local ptMember = partyMembers[i]\n            if i == 1 or i == 2 then\n                GUI:ColorButton(\"#jobPos\", 0, 0, 255, 255, GUI.ColorEditMode_NoAlpha, 20, 20)\n            elseif i == 3 or i == 4 then\n                GUI:ColorButton(\"#jobPos\", 0, 255, 0, 255, GUI.ColorEditMode_NoAlpha, 20, 20)\n            else\n                GUI:ColorButton(\"#jobPos\", 255, 0, 0, 255, GUI.ColorEditMode_NoAlpha, 20, 20)\n            end\n            GUI:SameLine()\n            if i == 1 or i == 2 then\n                GUI:TextColored(0, 100, 255, 1, \"[\" .. ptMember.label .. \"]\")\n            elseif i == 3 or i == 4 then\n                GUI:TextColored(0, 255, 0, 1, \"[\" .. ptMember.label .. \"]\")\n            else\n                GUI:TextColored(255, 0, 0, 1, \"[\" .. ptMember.label .. \"]\")\n            end\n            GUI:SameLine()\n            if ptMember.info ~= nil and ptMember.info.job ~= nil and ptMember.info.name ~= nil then\n                GUI:Selectable(\n                    \"   \" .. MuAiGuide.GetJobNameById(ptMember.info.job) .. \"      \" .. ptMember.info.name,\n                    IsSelected()\n                )\n            else\n                GUI:Selectable(\"   \" .. \"未知\" .. \"      \" .. \"未知玩家\" .. i, IsSelected())\n            end\n            if\n                GUI:IsItemHovered(\n                    GUI.HoveredFlags_AllowWhenBlockedByPopup + GUI.HoveredFlags_AllowWhenBlockedByActiveItem +\n                    GUI.HoveredFlags_AllowWhenOverlapped\n                )\n            then\n                if GUI:IsMouseDown(0) then\n                    if MuAiGuide.Party.mousePosition == nil then\n                        if MuAiGuide.Party.mousePosition ~= ptMember.label then\n                            MuAiGuide.Party.mousePosition = ptMember.label\n                        end\n                        if MuAiGuide.Party.selected ~= ptMember.label then\n                            MuAiGuide.Party.selected = ptMember.label\n                        end\n                    elseif MuAiGuide.Party.mousePosition ~= ptMember.label then\n                        d(\"[MuAiGuide]站位交换：\" .. MuAiGuide.Party.mousePosition .. \"<==>\" .. ptMember.label)\n                        local temp = MuAiGuide.Party[MuAiGuide.Party.mousePosition]\n                        MuAiGuide.Party[MuAiGuide.Party.mousePosition] = MuAiGuide.Party[ptMember.label]\n                        MuAiGuide.Party[ptMember.label] = temp\n\n                        -- 职能调整\n                        if MuAiGuide.Party[MuAiGuide.Party.mousePosition].id == MuAiGuide.GetPlayer().id then\n                            MuAiGuide.SelfPos = MuAiGuide.Party.mousePosition;\n                        elseif MuAiGuide.Party[ptMember.label].id == MuAiGuide.GetPlayer().id then\n                            MuAiGuide.SelfPos = ptMember.label;\n                        end\n\n                        MuAiGuide.Party.mousePosition = ptMember.label\n                        if MuAiGuide.Party.selected ~= ptMember.label then\n                            MuAiGuide.Party.selected = ptMember.label\n                        end\n                    end\n                end\n            end\n\n            if MuAiGuide.Party.mousePosition ~= nil and (GUI:IsMouseReleased(0) or not GUI:IsMouseDown(0)) then\n                MuAiGuide.Party.mousePosition = nil\n            end\n        end\n        GUI:ListBoxFooter()\n        if\n            MuAiGuide.Party.mousePosition ~= nil and\n            not GUI:IsItemHovered(\n                GUI.HoveredFlags_AllowWhenBlockedByPopup + GUI.HoveredFlags_AllowWhenBlockedByActiveItem +\n                GUI.HoveredFlags_AllowWhenOverlapped\n            )\n        then\n            MuAiGuide.Party.mousePosition = nil\n        end\n        GUI:Dummy(1, 1)\n        GUI:Button(\"绝伊甸设置\")\n        if GUI:IsItemClicked(0) then\n            MuAiGuide.ConfigFruInit = false\n            MuAiGuide.FruConfigUI.open = not MuAiGuide.FruConfigUI.open\n        end\n\n        GUI:Dummy(1, 3)\n        local winPosx, winPosy = GUI:GetWindowPos();\n        MuAiGuide.FruConfigUI.x = winPosx + 300\n        MuAiGuide.FruConfigUI.y = winPosy\n    end\n    GUI:SetWindowSize(300, 0)\n    GUI:End()\nend\nself.used = true\n",
						conditions = 
						{
